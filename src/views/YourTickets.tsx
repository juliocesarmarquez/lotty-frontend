...
export default function YourTickets({
  ...
  const handleBuy = async (
    ...
    const newTicket: TicketRecord = {
      ...
      quantity,
      purchaseDate: new Date().toLocaleDateString(i18n.language, { dateStyle: 'short' }),
    ...
    <div className="space-y-4 pb-24">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="neo-button flex items-center gap-2 py-2 px-4"
          data-testid="tickets-buy-button">
          <Plus size={16} />
          <span>{t('tickets.buy')}</span>
        </button>
      </div>
      ...
      {tickets.length === 0 ? (
        <div className="neo-card text-center py-8">
          <Image
            ...
            alt={t('tickets.buyAlt')}
            width={80}
            height={80}
            className="mx-auto mb-3"
          />
          <p className="font-display font-bold">{t('tickets.noTickets')}</p>
          <p className="text-sm text-text-main/60 mt-1">
            {t('tickets.buyToEnter')}
          </p>
        </div>
      ) : (
      ...
      <div className="neo-card flex items-start gap-3">
        ...
        <div className="text-sm text-text-main/70">
          <p className="font-display font-bold text-text-main mb-1">
            {t('tickets.howItWorks')}
          </p>
          <p>
            {t('tickets.howItWorksDesc', { price: ticketPrice })}
          </p>
        </div>
      </div>
